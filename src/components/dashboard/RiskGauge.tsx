import { formatProbability } from "../../lib/metrics";
...
export function RiskGauge({ probability, band }: { probability: number; band: string }) {
  ...
  // A calibrated model that says 99.93% is still not certain. Rendering "100%"
  // reads as a guarantee and undermines every calibration claim on the page,
  // so the dial is clamped and the raw score is shown alongside it.
  const clamped = formatProbability(probability);
  return (
      ...
      <div className="mt-5 flex items-center gap-6">
        ...
        >
          <div className="grid h-24 w-24 place-items-center rounded-full bg-slate-900 text-center">
            <span>
              <strong className="block text-3xl text-white">{clamped}</strong>
        ...
        <div className="min-w-0">
          ...
          <p className="mt-2 text-xs leading-relaxed text-slate-400">
            A calibrated score used by policy and ranking—not a guarantee of abandonment.
          </p>
          <p className="mt-2 font-mono text-[11px] text-slate-600">
            raw {probability.toFixed(4)} · displayed capped at 99%
          </p>
        </div>
      </div>
    </section>
