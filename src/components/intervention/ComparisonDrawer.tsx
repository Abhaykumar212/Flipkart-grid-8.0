...
export function ComparisonDrawer({ intervention, reasons, onClick, onDismiss }: InterventionSurfaceProps) {
  ...
  return (
    <aside className="fk-intervention fixed inset-x-2 bottom-2 z-30 mx-auto max-w-2xl rounded-[2px] border border-fk-border bg-white p-4 pl-5 shadow-fk-hover sm:inset-x-auto sm:right-3 sm:w-[440px]" role="status" aria-live="polite" data-testid="intervention-comparison-drawer">
      <div className="flex items-start gap-3"><GitCompareArrows className="mt-0.5 h-5 w-5 text-fk-blue" /><div className="min-w-0 flex-1"><p className="font-medium text-fk-ink">{intervention.headline ?? "Compare before you decide"}</p><p className="mt-1 text-fk-base text-fk-muted">{intervention.body ?? intervention.reason}</p><InterventionWhy reasons={reasons} /></div><button type="button" onClick={onDismiss} className="flex h-11 w-11 items-center justify-center text-fk-muted" aria-label="Dismiss comparison"><X className="h-5 w-5" /></button></div>
