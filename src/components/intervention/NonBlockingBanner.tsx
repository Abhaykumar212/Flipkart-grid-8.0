...
export function NonBlockingBanner({ intervention, onClick, onDismiss }: InterventionSurfaceProps) {
  return (
    <section className="mx-auto flex w-full max-w-fk items-center gap-3 border-b border-fk-border bg-blue-50 px-4 py-3" role="status" aria-live="polite" data-testid="intervention-banner">
      ...
      <p className="min-w-0 flex-1 text-fk-base text-fk-ink"><strong>{intervention.headline ?? intervention.display_name}</strong> {intervention.body ?? intervention.reason}</p>
